...
phenologyForecast <- function(siteName,URL,forecastLength){
  ...
  if(!continue){
    ...
    #retJSON <- {"body": {"xlab" : "Time", "ylab": "Mean GCC", "x": time , "y": phenoData$gcc_mean}}
    #retJSON <- toJSON(retJSON)
    dev.off()
    print("\n")
    ##SAVE TO TEXT FILE THEN READ THAT WITH WRAPPER
    retJSON <- paste(
      "{\"body\": {
        \"xlab\": \"Time\",\n \"ylab\": \"Mean GCC\",
    ...
    #cat(retJSON)
    #jsontest = toJSON(phenoData$gcc_mean)
    #cat(jsontest)
    #cat("{\"msg\": \"Hello from arbitrary C program!\"}\n")
