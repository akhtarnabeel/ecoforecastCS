...
use EcoForecast
```
Right now we are using two main tables, users: to store user related data and results: to store data for each experiment a user runs in our system (time, name, results etc.). Create these tables by running following (It is easy to manipulate MongoDB with some UI base client such as [https://www.mongodb.com/products/compass] Compass)
```
users
results
```
