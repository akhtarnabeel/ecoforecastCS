...
#output
    ...
    retJSON <- paste(
      "{\"body\": {
        \"Time\":", paste(toJSON(timeForecast)), 
      ",\n\"Observed\":", paste(toJSON(paste0(c(phenoData$gcc_mean, rep(NA, times = forecastLength))))), 
      ",\n\"Predicted\":", paste(toJSON(ci[2,])), 
      ",\n\"Lower\":", paste(toJSON(ci[1,])),
      ",\n\"Upper\":", paste(toJSON(ci[3,])), "\n}\n}"
    )    
